...
impl<T: PartialOrd> LinkedList<T> {
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        let mut h1 = list_a.start;
        let mut h2 = list_b.start;
        while let (Some(node1_ptr), Some(node2_ptr)) = (h1, h2) {
            let v1 = unsafe { &node1_ptr.as_ref().val };
            let v2 = unsafe { &node2_ptr.as_ref().val };
